...
def get_filters():
    ...
    Returns:
        ...
        (str) month - name of the month to filter by, or "all" to apply all months
        (str) day - name of the day of week to filter by, or "all" to apply all days of week
    """
...
def user_stats(df,city):
    ...
    # TO DO: Display oldest, youngest, and most common year of birth
...
def display_data(df):
    a=0
    answer = input('Please type "Yes" if you would like to view the first 5 list of data\n').lower()
    while True:
        if answer == 'no':
            break
        print(df[a:a+5])
